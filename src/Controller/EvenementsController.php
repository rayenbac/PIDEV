...
class EvenementsController extends AbstractController
{
    ...
    public function afficheE(EvenementsRepository $repository, SessionInterface $session): Response
    {
        $e = $this->getDoctrine()->getRepository(Evenements::Class);
        //utiliser la fonction findAll()
        $e = $repository->findAll();
        foreach ($e as $evenement) {
            if ($evenement->getNbrdeplaces() == 0) {
                $session->getFlashBag()->add('warning', 'pas de places pour cet evenement');
            }
        }
